...
import { createOrderSchema, orderIdSchema } from "../schema/order.schemas.js";
import {
  createOrder,
  getOrdersByUserId,
  getOrderById,
...
export const previewOrderPricingHandler: RequestHandler = catchErrors(
  async (req, res) => {
    // We can reuse createOrderSchema but omit things if we want.
    // For now, let's just parse it directly since preview has same shape.
    const data = createOrderSchema.parse(req.body);
    const userId = req.userId as number;

    const pricing = await previewOrderPricing(userId, data);
...
export const getAdminOrderDetailHandler: RequestHandler = catchErrors(
  async (req, res) => {
    const { id } = orderIdSchema.parse(req.params);
