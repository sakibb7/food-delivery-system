...
import {
  ...
  orderStatusSchema,
  orderIdSchema,
} from "../schema/restaurant.schemas.js";
import {
  createRestaurant,
  updateRestaurant,
  deleteRestaurant,
...
export const getMyRestaurantByIdHandler: RequestHandler = catchErrors(
  async (req, res) => {
    const { id } = restaurantIdSchema.parse(req.params);
    const userId = req.userId as number;
    ...
    const restaurant = await getRestaurantById(Number(id));
    appAssert(restaurant, NOT_FOUND, "Restaurant not found");
    appAssert(restaurant.ownerId === userId, FORBIDDEN, "Not authorized to access this restaurant");
...
export const updateRestaurantHandler: RequestHandler = catchErrors(
  async (req, res) => {
    ...
    const updatedRestaurant = await updateRestaurant(Number(id), cleanData as any);
...
export const markOrderSelfDeliveredHandler: RequestHandler = catchErrors(
  async (req, res) => {
    const { id } = restaurantIdSchema.parse(req.params);
    const { orderId } = orderIdSchema.parse(req.params);
    const userId = req.userId as number;

    const updatedOrder = await markOrderSelfDelivered(
      Number(id),
      Number(orderId),
      userId
    );
    ...
    return res.status(OK).json({
      message: "Order marked as self-delivered",
      order: updatedOrder,
    });
  }
);
...
export const adminGetRestaurantDetailsHandler: RequestHandler = catchErrors(
  async (req, res) => {
    const { id } = restaurantIdSchema.parse(req.params);

    const data = await getRestaurantDetailsForAdmin(Number(id));
    appAssert(data, NOT_FOUND, "Restaurant not found");
...
export const searchRestaurantsHandler: RequestHandler = catchErrors(
  async (req, res) => {
    const { q, city, area } = req.query as Record<string, string | undefined>;

    const filters: { q?: string; city?: string; area?: string } = {};
    if (q) filters.q = q;
...
export const getPopularCuisinesHandler: RequestHandler = catchErrors(
  async (_req, res) => {
    const cuisines = await getPopularCuisines();

    return res.status(OK).json({ cuisines });
  }
);

export const getCitiesWithCountsHandler: RequestHandler = catchErrors(
  async (_req, res) => {
