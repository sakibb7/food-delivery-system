...
export default function IncomingOrderScreen() {
  ...
  // Parse coordinates for the mini map preview
  const restaurantCoords = useMemo(() => {
    if (activeOrder?.restaurantLat && activeOrder?.restaurantLng) {
      return {
        latitude: parseFloat(activeOrder.restaurantLat),
        longitude: parseFloat(activeOrder.restaurantLng),
      };
    }
    return undefined;
  ...
  const customerCoords = useMemo(() => {
    if (activeOrder?.deliveryLat && activeOrder?.deliveryLng) {
      return {
        latitude: parseFloat(activeOrder.deliveryLat),
        longitude: parseFloat(activeOrder.deliveryLng),
      };
    }
    return undefined;
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-900" edges={["top"]}>
      {/* Mini Map Preview — shows restaurant + customer locations */}
      <View className="h-[35%] relative">
        <DeliveryMap
          ...
          showRider={true}
          restaurantName={activeOrder.restaurantName || "Restaurant"}
          customerLabel="Customer"
          enableLocationTracking={true}
        />
        {/* Gradient overlay at bottom of map */}
        <View className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-gray-900" style={{ backgroundColor: 'rgba(17,24,39,0.5)' }} />
      </View>
      ...
      <View className="flex-1 justify-center px-6">
        <View className="bg-white rounded-3xl w-full p-6 shadow-2xl relative overflow-hidden">
          ...
          <View className="items-center mb-5">
            <View className="bg-emerald-100 px-4 py-2 rounded-full mb-3 flex-row items-center">
