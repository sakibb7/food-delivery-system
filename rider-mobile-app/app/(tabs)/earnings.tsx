...
export default function EarningsScreen() {
  ...
  const { data: codData } = useGetQuery<any>({
    url: "/rider/cod/balance",
    queryKey: "rider-cod-balance",
  });

  const totalEarnings = data?.totalEarnings ?? 0;
  const deliveries = data?.deliveries ?? 0;
  const history = data?.history ?? [];
  const codBalance = codData?.codBalance ?? 0;
    ...
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      ...
      {isLoading ? (
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          {/* COD Cash Balance */}
          {codBalance > 0 && (
            <View className="mx-6 mb-6 bg-rose-500 p-5 rounded-2xl">
              <View className="flex-row items-center justify-between mb-2">
                <View className="flex-row items-center">
                  <Ionicons name="cash-outline" size={20} color="white" />
                  <Text className="text-rose-50 font-bold ml-2 text-base">COD Cash in Hand</Text>
                </View>
              </View>
              <Text className="text-white text-3xl font-bold mb-2">
                {currencySymbol}{Number(codBalance).toFixed(2)}
              </Text>
              <Text className="text-rose-100 text-sm">
                Please deposit this amount to the platform via bank transfer or cash handoff.
              </Text>
            </View>
          )}
