...
const mockOrders = [
  { id: "ORD-8739", restaurant: "Pizza Hut", date: "Today, 1:30 PM", items: "1x Large Math, 1x Coke", total: "$24.50", status: "Delivered", img: "/pizza.png", type: "Delivery" },
  { id: "ORD-8738", restaurant: "Burger King", date: "Yesterday, 7:45 PM", items: "2x Whopper, 2x Fries", total: "$18.00", status: "Delivered", img: "/burger.png", type: "Delivery" },
  { id: "ORD-8737", restaurant: "Sushi Samurai", date: "Oct 12, 8:15 PM", items: "1x Dragon Roll, 1x Miso Soup", total: "$32.00", status: "Cancelled", img: "/sushi.png", type: "Pickup" },
];
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full font-sans">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1 font-medium">View and track your previous and current orders.</p>
      </div>
      ...
      <div className="space-y-6">
        {mockOrders.map((order) => (
          <div key={order.id} className="bg-white border border-gray-100 rounded-3xl p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow group flex flex-col sm:flex-row sm:items-center gap-6">
            <div className="flex-shrink-0 w-24 h-24 rounded-2xl overflow-hidden relative border border-gray-100">
               <Image src={order.img} alt={order.restaurant} fill className="object-cover" />
            </div>
            ...
            <div className="flex-1 min-w-0">
               <div className="flex flex-col sm:flex-row justify-between mb-1 sm:items-center gap-2">
                 ...
                 <span className={`px-3 py-1 rounded-full text-xs font-bold w-fit ${order.status === 'Delivered' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                   {order.status}
                 </span>
               </div>
               ...
               <div className="pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-3">
                 <Link href={`/orders/${order.id}`} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-800 font-semibold rounded-xl transition-colors text-sm">
