...
export default function MyRestaurantsPage() {
  ...
  const { mutate: deleteRestaurant, isLoading: isDeleting } = useQueryMutation({
    url: "/restaurant",
    method: "DELETE",
  });
      ...
      {restaurants.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {restaurants.map((restaurant: Restaurant) => (
            <div key={restaurant.id} className="group bg-white rounded-3xl overflow-hidden border border-gray-100 flex flex-col h-full border-b-4 border-b-red-500">
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Link href={`/dashboard/restaurants/${restaurant.id}`} className="bg-white/90 backdrop-blur-sm p-2 rounded-xl shadow-lg cursor-pointer hover:bg-white transition-colors inline-flex">
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <div className="mt-auto grid grid-cols-3 gap-2 pt-6 border-t border-gray-100">
                  ...
                  <Link href={`/dashboard/restaurants/${restaurant.id}`} onClick={(e) => e.stopPropagation()} className="inline-flex flex-col items-center justify-center gap-1 rounded-xl font-bold border-2 border-gray-100 hover:border-red-100 hover:bg-red-50 text-gray-600 hover:text-red-600 transition-all p-2 text-sm text-center">
                    <Edit2 size={16} />
                    Manage
                  </Link>
                  <button onClick={(e) => handleDelete(e, restaurant.id, restaurant.name)} disabled={isDeleting} className="inline-flex flex-col items-center justify-center gap-1 rounded-xl font-bold border-2 border-gray-100 hover:border-red-100 hover:bg-red-50 text-gray-600 hover:text-red-600 transition-all p-2 text-sm cursor-pointer text-center disabled:opacity-50">
                    <Trash2 size={16} />
                    Delete
                  </button>
                </div>
