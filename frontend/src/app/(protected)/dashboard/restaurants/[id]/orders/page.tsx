...
export default function RestaurantOrdersPage() {
  ...
  const handleSelfDelivery = (orderId: number) => {
    mutate({
      updatedUrl: `/restaurant/${restaurantId}/orders/${orderId}/self-deliver`,
    }, {
      onSuccess: () => {
        refetch();
      }
  ...
  useEffect(() => {
    const pendingOrdersCount = activeOrders.filter((o: any) => o.status === "pending").length;
    if (pendingOrdersCount > prevOrderCount) {
    ...
    setPrevOrderCount(pendingOrdersCount);
  }, [activeOrders, prevOrderCount]);
      ...
      case "ready_for_pickup":
        return (
          <div className="flex items-center gap-3">
            ...
            <button disabled={isLoadingMutation} onClick={() => handleSelfDelivery(order.id)} className="px-4 py-2 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition">Mark Self Delivered</button>
          </div>
        );
