...
export default function Dashboard() {
  ...
  const { data: restaurantsData, isLoading: restaurantsLoading } = useGetQuery({
    url: "/restaurant",
    queryKey: "recommended_restaurants",
    enabled: !isOwner,
  });

  const { data: ordersData, isLoading: ordersLoading } = useGetQuery({
    url: "/order",
  ...
  const recommendedRestaurants = restaurantsData?.restaurants?.slice(0, 3) || [];
  ...
  if (isOwner) {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full font-sans">
        ...
        {/* Earnings Quick Card */}
        <Link href="/dashboard/earnings" className="block mb-8">
          <div className="bg-gradient-to-r from-emerald-500 to-teal-600 p-6 rounded-3xl text-white shadow-lg hover:shadow-xl transition-all hover:scale-[1.01] active:scale-[0.99]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-emerald-100 text-sm font-medium mt-0.5">View detailed earnings breakdown, commission reports, and payout history</p>
                </div>
              </div>
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {restaurantsLoading ? (
            ...
            <div className="col-span-full py-10 text-center text-gray-500 font-medium bg-white rounded-3xl border border-gray-100">
              No restaurants found. Please check back later!
            </div>
          ) : (
            recommendedRestaurants.map((item: any) => {
              const fav = isFavorite(item.id);
              const fallbackImg = "https://images.unsplash.com/photo-1514933651103-005eec06c04b?q=80&w=1974&auto=format&fit=crop";
                ...
                >
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        toggleFavorite({
                          id: item.id,
                          name: item.name,
                          tags: item.cuisine || "Various",
                          rating: Number(item.rating) || 4.5,
                          time: item.estimatedDeliveryTime || "30-45 min",
                          img: item.image || fallbackImg
                        });
                      }}
                      className={`absolute top-3 right-3 p-2 backdrop-blur-md rounded-full transition-all ${fav ? "bg-red-50 text-red-500" : "bg-white/80 text-gray-500 hover:text-red-500 hover:bg-white"
                        }`}
                    >
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex items-center gap-1 bg-green-50 text-green-700 px-2.5 py-1 rounded-lg text-sm font-bold shrink-0">
                        {Number(item.rating || 4.5).toFixed(1)} <Star size={14} fill="currentColor" />
                      </div>
                    </div>
                    ...
                    <div className="flex items-center text-sm font-medium text-gray-600 bg-gray-50 w-fit px-3 py-1.5 rounded-lg">
                      ...
                      {item.estimatedDeliveryTime || "30-45 min"}
                    </div>
                  </div>
                </Link>
              );
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
          {ordersLoading ? (
            <div className="p-8 text-center">
              <Loader2 className="animate-spin mx-auto text-gray-400 mb-2" size={32} />
                ...
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl overflow-hidden bg-gray-50 flex items-center justify-center shrink-0 border border-gray-100">
                    {order.restaurantLogo ? (
                      <img src={order.restaurantLogo} alt={order.restaurantName || "Restaurant"} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-2 truncate max-w-sm">
                      Order #{order.id} • {order.itemsCount || 0} items
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  <div className={`px-3 py-1 rounded-lg text-sm font-semibold flex-1 sm:flex-none text-center ${order.status === 'delivered' ? 'bg-green-50 text-green-700' :
