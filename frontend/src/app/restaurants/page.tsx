...
import { useRestaurants, InitialSearchParams } from "@/hooks/useRestaurants";
...
function RestaurantsContent() {
  ...
  // Read URL query params for initial search
  const initialParams: InitialSearchParams = useMemo(() => ({
    q: searchParams.get("q") || undefined,
    city: searchParams.get("city") || undefined,
    cuisine: searchParams.get("cuisine") || undefined,
  }), [searchParams]);

  const { restaurants, allRestaurants, loading, error, filters, updateFilters } = useRestaurants(initialParams);
  ...
  // Build a dynamic page title based on search context
  const pageTitle = useMemo(() => {
    if (initialParams.q && initialParams.city) {
      return `"${initialParams.q}" in ${initialParams.city}`;
    }
    if (initialParams.q) {
      return `Results for "${initialParams.q}"`;
    }
    if (initialParams.city) {
      return `Restaurants in ${initialParams.city}`;
    }
    if (initialParams.cuisine) {
      return `${initialParams.cuisine} Restaurants`;
    }
    return "Best Food Near You";
  }, [initialParams]);
