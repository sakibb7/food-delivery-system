...
export default function Support() {
  ...
  const { data, isLoading, refetch } = useGetQuery<{ tickets: Ticket[] }>({
    url: "/support/admin/all",
    queryKey: ["admin-support-tickets", queryParams as any],
    params: queryParams,
  });
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex gap-6 overflow-hidden">
      ...
      {selectedTicket && (
        <div className={`w-full lg:w-[500px] shrink-0 bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col overflow-hidden`}>
          {/* Header */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-gray-900 flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${selectedTicket.status === 'resolved' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                    }`}>
            ...
            {isLoadingDetail ? (
              ...
              selectedTicket.messages?.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.senderType === 'staff' ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`p-3 max-w-[85%] rounded-2xl text-sm ${msg.senderType === 'staff'
