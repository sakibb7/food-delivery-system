...
const NAVIGATION = [
  ...
  { name: "Zones", href: "/zones", icon: Map, permission: "manage_settings" },
];
        ...
        <header className="h-16 bg-white border-b border-gray-200 sticky top-0 z-10 flex items-center justify-between px-4 sm:px-6">
          <div className="flex items-center flex-1">
            ...
            <div className="hidden sm:flex items-center ml-4 flex-1">
              {/* Future: Global search can be implemented here */}
            </div>
          </div>
          ...
          <div className="ml-4 flex items-center space-x-4">
            ...
            <Link to="/profile" className="flex items-center gap-3 pl-4 border-l border-gray-200 hover:bg-gray-50 p-2 -mr-2 rounded-lg transition-colors cursor-pointer group">
              <div className="h-8 w-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-semibold text-sm group-hover:bg-orange-200 transition-colors">
                {user?.firstName?.charAt(0) || "A"}
              </div>
              <div className="hidden sm:block text-left">
                <p className="text-sm font-medium text-gray-700 group-hover:text-orange-600 transition-colors">{user?.firstName || "Admin User"}</p>
                <p className="text-xs text-gray-500 capitalize">{user?.adminRole?.name || user?.role || "Admin"}</p>
              </div>
            </Link>
          </div>
        </header>
